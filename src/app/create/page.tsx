...
export default function CreatePage() {
  ...
  const onSave = () => { setShowRealisation(true); };

  // Compute lead score based on engagement
  let score = 0;
  if (leadSubmitted) score += 10;
  if (showRealisation) score += 15;
  if (name) score += 5;

  const crestForRealisation = { bg: { hex: bg.hex }, outline: { hex: outline.hex }, inner: { hex: inner.hex }, name };
  ...
  return (
    <section className="py-16 md:py-24">
      <AnimatePresence mode="wait">
        {showRealisation && (
          <Realisation key="real" crest={crestForRealisation} onClose={() => setShowRealisation(false)} />
        )}
      </AnimatePresence>
        ...
        {step === 0 && !leadSubmitted && (
          <div className="max-w-[560px] mx-auto text-center py-12">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              ...
              <form onSubmit={(e) => { e.preventDefault(); if (leadForm.email && leadForm.consent) { setLeadSubmitted(true); setStep(1); }}}
              ...
              <div className="bg-white rounded-2xl border border-[rgba(10,22,40,0.08)] p-6">
                ...
                <div className={`flex items-center justify-center rounded-xl p-8 ${bgCtx}`}>
                  <motion.div layout className="w-full max-w-[320px]">
                    <Crest bg={bg.hex} outline={outline.hex} inner={inner.hex} label="PAPA PASTA" sub={name ? name.toUpperCase() : "EST. 2025"} className="w-full" />
                  </motion.div>
                </div>
              </div>
